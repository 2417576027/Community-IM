...
algorithms = ['celfpp']                   # ['heuristic','celfpp','ccelfpp1','genetic','cofim']
...
n_sim = 10                              # number of diffusions 原值：1000
...
all_algorithms = ['celfpp','ccelfpp1','degree','degdiscount']    # all algorithms['celfpp','celfpp1','genetic','cofim','degree','degdiscount']
...
if __name__ == '__main__':
    if run_algos:
        ...
        main_offline(name_id, graph_type, max_budget, algorithms, heuristics,
                     diffusion_models, is_graph_already_weighted, weighting_schemes,
                     n_sim, communities, community_methods, community_size_threshold)

    if evaluation:
        "evaluating different algorithms"
        main_evaluation(name_id, graph_type, max_budget, diffusion_models,
                        is_graph_already_weighted, weighting_schemes, num_procs,
                        interval, all_algorithms)

    if compilation:
        "compiling and plotting results for different algorithms"
        main_compilation(name_id, graph_type, max_budget, diffusion_models, weighting_schemes,
                              interval, all_algorithms)
    if summarization:
        "tabulating run-times and max expected influences for different algorithms all networks"
        main_summarization(max_budget, diffusion_models, weighting_schemes,
                                interval, all_algorithms, all_name_ids)